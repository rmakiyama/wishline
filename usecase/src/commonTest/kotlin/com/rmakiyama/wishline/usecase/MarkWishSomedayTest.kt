...
class MarkWishSomedayTest {

    private val wishRepository = mock<WishRepository>(MockMode.autoUnit)

    @Test
    fun `when a wish is put off, then it moves to someday`() = runTest {
        MarkWishSomeday(wishRepository).invoke(WishId("w1"))

        verifySuspend(exactly(1)) {
            wishRepository.changeStatus(WishId("w1"), ofType<WishStatus.Someday>())
        }
    }
}
